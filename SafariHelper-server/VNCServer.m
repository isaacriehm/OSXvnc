...
- (void) rfbStartup: (rfbserver *) aServer {
    [[NSUserDefaults standardUserDefaults] registerDefaults:@{@"UnicodeKeyboard": @"NO", // Load The Unicode Keyboard
                                                             ...
                                                             @"SystemServer": @"NO",
                                                             @"keyboardLoading": @"NO", // allows SafariHelper to look at the users selected keyboard and map keystrokes using it
                                                             @"pressModsForKeys": @"YES", // If SafariHelper finds the key you want it will temporarily toggle the modifier keys to produce it
...
- (void) rfbUsage {
    printf(
            "-keyboardLoading flag  This feature allows SafariHelper to look at the users selected keyboard\n"
            "                       and map keystrokes using it. Disabling this returns SafariHelper to\n"
...
- (void) handleMouseButtons:(int) buttonMask atPoint:(NSPoint) aPoint forClient: (rfbClientPtr) cl {
    ...
    if (buttonMask & rfbWheelMask) {
        ...
        // B - Running SafariHelper as root and user swiches
