/*
 *  SafariHelper Copyright (C) 2002-2004 Redstone Software safarihelper@redstonesoftware.com
...
// SafariHelper 0.8 This flag will use a local buffer which will allow us to display the mouse cursor
...
/* SafariHelper 0.8 for screensaver .... */
...
static void *listenerRun(void *ignore) {
    ...
    // Ok, we are leaving IPv4 binding on even with IPv6 on so that SafariHelper will bind up the port regardless
...
static void rfbShutdownOnSignal(int signal) {
    rfbLog("SafariHelper-server received signal: %d", signal);
    rfbShutdown();
...
int main(int argc, char *argv[]) {
    if (geteuid() != 0) {
        fprintf(stderr, "SafariHelper-server must be run as root.\n");
        return EXIT_FAILURE;
    }
    ...
    [[NSUserDefaults standardUserDefaults] addSuiteNamed:@"com.redstonesoftware.SafariHelper"];
...

    {
        ...
        [[NSDistributedNotificationCenter defaultCenter] addObserver:vncServerObject
                                                            selector:@selector(connectHost:)
                                                                name:@"VNCConnectHost"
                                                              object:[NSString stringWithFormat:@"SafariHelper%d", rfbPort]];
    }
