...
/*
 *  SafariHelper Copyright (C) 2001 Dan McGuirk <mcguirk@incompleteness.net>.
...
#define SafariHelperRemoteProxy @"SafariHelperRemoteProxy"
...
- (void) setupTypes:(NSArray *) availableTypes {
	...
	NSInteger newChangeCount = [thePasteboard declareTypes:[availableTypes arrayByAddingObject:SafariHelperRemoteProxy] owner:self];
		...
		if ([availableType hasPrefix:@"RSFileWrapper:"]) { // special file data
			...
			if ([availableType isEqualToString:NSFileContentsPboardType]) {
				...
				if (!clientPointer->receivedFileTempFolder) {
					clientPointer->receivedFileTempFolder = [[@"/tmp/SafariHelper-" stringByAppendingString:[NSProcessInfo processInfo].globallyUniqueString] retain];
...
// Currently there is a problem when SafariHelper is run PRIOR to the pbs (which starts when a user logs in)
// the SafariHelper process is NOT connected to the pbs port - this is an OS X security measure which we aren't certain
// how to work around
...
static BOOL pasteboardRepresentsExistingFile(NSPasteboard *thePasteboard) {
	...
	else if ([pboardTypes containsObject:SafariHelperRemoteProxy]) {
